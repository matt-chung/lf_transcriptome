---
title: "B. malayi transcriptome v2"
...
```{r setinputs}
counts.path <- "X:/mattchung/EBMAL/bmalayi_counts.tsv"
tpm.path <- "X:/mattchung/EBMAL/bmalayi_tpm.tsv"
groups.path <- "X:/mattchung/EBMAL/bmalayi_groups.tsv"
geneinfo.path <- "X:/mattchung/EBMAL/bmalayi_gene.info"
output_dir <- "Z:/EBMAL/mchung_dir/bmalayi_transcriptome"

# counts.path <- "Z:/EBMAL/mchung_dir/wbm_transcriptome/wbm_counts.tsv"
# tpm.path <- "Z:/EBMAL/mchung_dir/wbm_transcriptome/wbm_tpm.tsv"
...
```{r bromodomain_heatmap, fig.height = 3.2, fig.width = 5}
bromodomain.genes <- geneinfo$gene[grep("IPR001487",geneinfo$interpro_description)]
bromodomain.zscore.log2tpm.de <- zscore.log2tpm.de[rownames(zscore.log2tpm.de) %in% bromodomain.genes,]

rowcol1 <- tpm.de.wgcna$module[rownames(tpm.de.wgcna) %in% bromodomain.genes]
rowcol2 <- tpm.de.wgcna$invert[rownames(tpm.de.wgcna) %in% bromodomain.genes]
rowcol2[rowcol2 == F]<- "grey"
rowcol2[rowcol2 == T]<- "black"

rowsep <- get_heatmap_separators(rowcol1)
pdf(paste0(output_dir,"/bromodomain_zscorelog2tpm_heatmap.pdf"),
    height=3.2,
    width=5)
heatmap.2.fix(as.matrix(bromodomain.zscore.log2tpm.de),
              col=hmcol,
              trace="none",
              labRow=gsub("Gene:","",rownames(bromodomain.zscore.log2tpm.de)),
              Rowv = F,
              Colv = F,
              RowSideColors=rowcol1,
              ColSideColors=colcol,
...
## bromodomain - lineplot

```{r bromodomain_lineplot, , fig.height = 4, fig.width = 6}
bromodomain.zscore.log2tpm.de.melt <- melt(t(bromodomain.zscore.log2tpm.de))
bromodomain.zscore.log2tpm.de.melt[,1] <- factor(bromodomain.zscore.log2tpm.de.melt[,1],levels=groups[,1])
bromodomain.zscore.log2tpm.de.melt[,4] <- bromodomain.zscore.log2tpm.de.melt[,2]
for(i in 1:length(rownames(bromodomain.zscore.log2tpm.de))){
  bromodomain.zscore.log2tpm.de.melt[,4] <- gsub(rownames(bromodomain.zscore.log2tpm.de)[i],
                                                 rowcol1[i],
                                                 bromodomain.zscore.log2tpm.de.melt[,4])
}

avgtrend <- as.data.frame(colMeans(bromodomain.zscore.log2tpm.de))
sem <- c()
for(i in 1:ncol(bromodomain.zscore.log2tpm.de)){
  sem[i] <- sd(bromodomain.zscore.log2tpm.de[,i])/sqrt(length(bromodomain.zscore.log2tpm.de[,i]))
}

bromodomain_lineplot <- ggplot()+
  geom_line(aes(x=bromodomain.zscore.log2tpm.de.melt[,1],
                y=bromodomain.zscore.log2tpm.de.melt[,3],
                group = bromodomain.zscore.log2tpm.de.melt[,2]),
                color=bromodomain.zscore.log2tpm.de.melt[,4],
            alpha=0.5)+
  geom_line(aes(x=rownames(avgtrend), y=avgtrend[,1], group = 1), color = "black")+
  geom_point(aes(x=rownames(avgtrend), y=avgtrend[,1], group = 1), color = "black")+
  geom_errorbar(aes(x=rownames(avgtrend), ymin=avgtrend[,1] - sem, ymax=avgtrend[,1] + sem), color="black")+
  coord_cartesian(ylim=c(-3,3))+
  labs(x="",y="z-score of log2TPM")+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, vjust=0, hjust = 1))

pdf(paste0(output_dir,"/bromodomain_zscorelog2tpm_lineplot.pdf"),
    height=4,
    width=6)
print(bromodomain_lineplot)
dev.off()
